...
const home = await surfaces.buildAdminHomeSurface({ summary: { connectedUsers: 9, connectedNoProfile: 2, profileStartedUsers: 7, readyNotListed: 2, listedUsers: 5, pendingOlder24h: 2, deliveryIssues: 2, noIntroYet: 3, acceptedIntroUsers: 1, firstIntroUsers: 4, failedDeliveries: 2 } });
...
for (const callback of ['adm:ops:funnel:conn_noprofile', 'adm:ops:funnel:delivery_issue']) {
  if (!JSON.stringify(home.reply_markup).includes(callback)) throw new Error(`Home keyboard missing ${callback}`);
}
for (const callback of ['adm:ops:funnel:connected', 'adm:ops:funnel:ready_not_listed']) {
