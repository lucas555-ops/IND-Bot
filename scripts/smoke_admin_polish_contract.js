...
const home = await surfaces.buildAdminHomeSurface({ summary: { totalUsers: 128, connectedUsers: 100, profileStartedUsers: 90, readyProfiles: 70, readyNotListed: 11, listedUsers: 42, listedActiveUsers: 33, noIntroYet: 19, firstIntroUsers: 17, acceptedIntroUsers: 9, failedDeliveries: 3, activeNotice: true, latestBroadcastStatus: 'sent_with_failures' } });
for (const fragment of ['👑 Админка', 'Пользователи всего: 128', 'Готовые, но не опубликованы: 11', 'Уведомление: активно']) {
...
for (const fragment of ['🧰 Операции', 'Опубликованы неполные: 2', 'Проблемы доставки: 4']) {
...
for (const fragment of ['В этом сегменте сейчас нет пользователей.', 'В этом quality-сегменте сейчас нет профилей.', 'В этом сегменте аудита пока нет событий.']) {
